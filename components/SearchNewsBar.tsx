...
export default function SearchNewsBar({
  categories,
  initialQ,
  initialCategoryId,
}: {
  ...
  const [suggest, setSuggest] = React.useState<{ title: string; slug: string }[]>(
    [],
  );
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`/api/news-suggest?${p.toString()}`, { cache: 'no-store' });
        if (!res.ok) return;
        const json = (await res.json()) as { items: { title: string; slug: string }[] };
  ...
  function applyFilter(next?: { q?: string; categoryId?: string; page?: number }) {
    ...
    router.push(`/tin-tuc?${qs.toString()}`);
  }
  ...
  return (
    <div ref={boxRef} className="w-full">
      ...
      >
        ...
        >
          <span className="hidden md:inline">Chuyên mục</span>
          ...
          {q ? (
            <button
              ...
              onClick={() => {
                ...
                setSuggest([]);
                setShowSuggest(false);
                // giữ nguyên category khi clear text
              }}
              ...
              {suggest.map((s) => (
                <li key={s.slug}>
                  <button
                    type="button"
                    onClick={() => {
                      // đi thẳng tới bài viết
                      router.push(`/tin-tuc/${s.slug}`);
                      setShowSuggest(false);
                    }}
